...
import { ArticlePurchaseQuotationEntry, Currency, Tax } from '@/types';
...
interface PurchaseQuotationTaxEntriesProps {
  className?: string;
...
export const PurchaseQuotationTaxEntries: React.FC<PurchaseQuotationTaxEntriesProps> = ({
  className,
  article,
  taxes,
  selectedTaxIds,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.articlePurchaseQuotationEntryTaxes ||
        article?.articlePurchaseQuotationEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.articlePurchaseQuotationEntryTaxes?.map((appliedTax, i) => (
