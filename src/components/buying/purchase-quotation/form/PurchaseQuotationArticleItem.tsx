...
import { ArticlePurchaseQuotationEntry, Currency, PurchaseQuotationTaxEntry, Tax } from '@/types';
import {
  Select,
  SelectTrigger,
  SelectContent,
...
import { PurchaseQuotationTaxEntries } from './PurchaseQuotationTaxEntries';


interface PurchaseQuotationArticleItemProps {
  className?: string;
  article: ArticlePurchaseQuotationEntry;
  onChange: (item: ArticlePurchaseQuotationEntry) => void;
...
export const PurchaseQuotationArticleItem: React.FC<PurchaseQuotationArticleItemProps> = ({
  className,
  article,
  onChange,
  taxes,
  ...
  const handleTaxChange = (value: string, index: number) => {
    ...
    const updatedTaxes = [...(article.articlePurchaseQuotationEntryTaxes || [])];
    if (selectedTax) {
      updatedTaxes[index] = { tax: selectedTax };
    } else {
      updatedTaxes.splice(index, 1);
    }
    onChange({ ...article, articlePurchaseQuotationEntryTaxes: updatedTaxes });
  };

  const handleTaxDelete = (index: number) => {
    const updatedTaxes = article.articlePurchaseQuotationEntryTaxes?.filter((_, i) => i !== index);
    onChange({ ...article, articlePurchaseQuotationEntryTaxes: updatedTaxes });
  };
  ...
  const selectedTaxIds = article.articlePurchaseQuotationEntryTaxes?.map((t) => t.tax?.id) || [];
                ...
                article.article?.description && (
                  <>
                    ...
                    <Textarea
                      ...
                      className="resize-none"
                      onClick={() => {}}
                      rows={3 + (article?.articlePurchaseQuotationEntryTaxes?.length || 0)}
                    />
                  </>
                )
              )}
