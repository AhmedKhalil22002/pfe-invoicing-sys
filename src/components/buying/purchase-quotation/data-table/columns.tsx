...
export const getPurchaseQuotationColumns = (
  t: Function,
  router: NextRouter,
  firmId?: number,
  interlocutorId?: number
): ColumnDef<PurchaseQuotation>[] => {
  const translationNamespace = 'invoicing';
  const translate = (value: string, namespace: string = '') => {
    return t(value, { ns: namespace || translationNamespace });
  };
  const firmColumn: ColumnDef<PurchaseQuotation> = {
    accessorKey: 'firm',
    header: ({ column }) => (
  ...
  const interlocutorColumn: ColumnDef<PurchaseQuotation> = {
