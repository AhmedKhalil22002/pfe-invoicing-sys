...
interface PurchaseQuotationInvoiceDialogProps {
  className?: string;
  id?: number;
...
export const PurchaseQuotationInvoiceDialog: React.FC<PurchaseQuotationInvoiceDialogProps> = ({
  className,
  id,
  status,
  sequential,
  ...
  const footer = (
    <div className="flex gap-2 mt-2 items-center justify-center">
      <Button
        ...
        onClick={() => {
          if (id) invoice(id, status != PURCHASE_QUOTATION_STATUS.Invoiced ? !invoiceMark : true);
          onClose();
        }}>
        <Check /> Facturer
      ...
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className={cn('max-w-[25vw] p-8', className)}>
          <DialogHeader>
            ...
            {status != PURCHASE_QUOTATION_STATUS.Invoiced && (
