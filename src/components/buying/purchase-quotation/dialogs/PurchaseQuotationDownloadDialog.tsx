...
interface PurchaseQuotationDownloadDialogProps {
  className?: string;
  id: number;
  open: boolean;
  downloadPurchaseQuotation: (template: string) => void;
  isDownloadPending: boolean;
  onClose: () => void;
}

export const PurchaseQuotationDownloadDialog: React.FC<PurchaseQuotationDownloadDialogProps> = ({
  className,
  id,
  open,
  downloadPurchaseQuotation,
  isDownloadPending,
  onClose
}) => {
  ...
  const body = (
    <div className={cn(className, 'grid grid-cols-2 gap-4')}>
      <div
        ...
        onClick={() => downloadPurchaseQuotation('template1')}>
        <File />
      ...
      <div
        className="flex gap-2 items-center cursor-pointer hover:bg-zinc-200 dark:hover:bg-zinc-500 rounded-lg p-4"
        onClick={() => downloadPurchaseQuotation('template2')}>
        <File />
      ...
      <div
        className="flex gap-2 items-center cursor-pointer hover:bg-zinc-200 dark:hover:bg-zinc-500 rounded-lg p-4"
        onClick={() => downloadPurchaseQuotation('template3')}>
        <File />
