...
interface PurchaseQuotationDuplicateDialogProps {
  className?: string;
  id: number;
  sequential: string;
  open: boolean;
  duplicatePurchaseQuotation: (includeFiles: boolean) => void;
  isDuplicationPending?: boolean;
  onClose: () => void;
}

export const PurchaseQuotationDuplicateDialog: React.FC<PurchaseQuotationDuplicateDialogProps> = ({
  className,
  id,
  sequential,
  open,
  ...
  const content = (
    <div className="flex gap-2 items-center">
      ...
      <Label>{tInvoicing('purchase-quotation.file_duplication')}</Label>
    </div>
  );
  ...
  const footer = (
    <div className="flex gap-2 mt-2 items-center justify-center">
      <Button
        ...
        onClick={() => {
          duplicatePurchaseQuotation(includeFiles);
