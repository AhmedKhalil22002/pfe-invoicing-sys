import { ArticlePurchaseQuotationEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type PurchaseQuotationPseudoItem = { id: string; article: ArticlePurchaseQuotationEntry & { total?: number } };

export type PurchaseQuotationArticleManager = {
  articles: PurchaseQuotationPseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticlePurchaseQuotationEntry) => void;
  update: (id: string, article: ArticlePurchaseQuotationEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticlePurchaseQuotationEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticlePurchaseQuotationEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForPurchaseQuotation = (article: ArticlePurchaseQuotationEntry) => {
  ...
  if (article?.articlePurchaseQuotationEntryTaxes) {
    for (const entry of article?.articlePurchaseQuotationEntryTaxes) {
      if (entry?.tax?.isRate) {
...
export const usePurchaseQuotationArticleManager = create<PurchaseQuotationArticleManager>()((set, get) => ({
  articles: [],
  taxSummary: [],

  add: (article: ArticlePurchaseQuotationEntry = {}) => {
    const { subTotal, total } = calculateForPurchaseQuotation(article);
  ...
  update: (id: string, article: ArticlePurchaseQuotationEntry) => {
    const { subTotal, total } = calculateForPurchaseQuotation(article);
  ...
  setArticles: (articles: ArticlePurchaseQuotationEntry[]) => {
    set({
  ...
  getArticles: () => {
    return get().articles.map((item) => {
      const { subTotal, total } = calculateForPurchaseQuotation(item.article);
