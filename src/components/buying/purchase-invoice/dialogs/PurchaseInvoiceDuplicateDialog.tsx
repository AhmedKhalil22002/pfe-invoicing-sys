...
interface PurchaseInvoiceDuplicateDialogProps {
  className?: string;
  id: number;
  sequential: string;
  open: boolean;
  duplicatePurchaseInvoice: (includeFiles: boolean) => void;
  isDuplicationPending?: boolean;
  onClose: () => void;
}

export const PurchaseInvoiceDuplicateDialog: React.FC<PurchaseInvoiceDuplicateDialogProps> = ({
  className,
  id,
  sequential,
  open,
  ...
  const content = (
    <div className="flex gap-2 items-center">
      ...
      <Label>{tInvoicing('purchase_invoice.file_duplication')}</Label>
    </div>
  );
  ...
  const footer = (
    <div className="flex gap-2 mt-2 items-center justify-center">
      <Button
        ...
        onClick={() => {
          duplicatePurchaseInvoice(includeFiles);
