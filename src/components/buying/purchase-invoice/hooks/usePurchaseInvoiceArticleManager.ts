import { ArticlePurchaseInvoiceEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type PurchaseInvoicePseudoItem = { id: string; article: ArticlePurchaseInvoiceEntry & { total?: number } };

export type PurchaseInvoiceArticleManager = {
  articles: PurchaseInvoicePseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticlePurchaseInvoiceEntry) => void;
  update: (id: string, article: ArticlePurchaseInvoiceEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticlePurchaseInvoiceEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticlePurchaseInvoiceEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForPurchaseInvoice = (article: ArticlePurchaseInvoiceEntry) => {
  ...
  if (article?.articlePurchaseInvoiceEntryTaxes) {
    for (const entry of article?.articlePurchaseInvoiceEntryTaxes) {
      if (entry?.tax?.isRate) {
...
export const usePurchaseInvoiceArticleManager = create<PurchaseInvoiceArticleManager>()((set, get) => ({
  articles: [],
  taxSummary: [],

  add: (article: ArticlePurchaseInvoiceEntry = {}) => {
    const { subTotal, total } = calculateForPurchaseInvoice(article);
  ...
  update: (id: string, article: ArticlePurchaseInvoiceEntry) => {
    const { subTotal, total } = calculateForPurchaseInvoice(article);
  ...
  setArticles: (articles: ArticlePurchaseInvoiceEntry[]) => {
    set({
  ...
  getArticles: () => {
    return get().articles.map((item) => {
      const { subTotal, total } = calculateForPurchaseInvoice(item.article);
