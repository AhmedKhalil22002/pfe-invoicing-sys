...
import { ArticlePurchaseInvoiceEntry, ResponseCurrencyDto, Tax } from '@/types';
...
interface PurchaseInvoiceTaxEntriesProps {
  className?: string;
...
export const PurchaseInvoiceTaxEntries: React.FC<PurchaseInvoiceTaxEntriesProps> = ({
  className,
  article,
  taxes,
  selectedTaxIds,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.articlePurchaseInvoiceEntryTaxes || article?.articlePurchaseInvoiceEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.articlePurchaseInvoiceEntryTaxes?.map((appliedTax, i) => (
