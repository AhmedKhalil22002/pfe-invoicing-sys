...
import { ResponseCurrencyDto, PURCHASE_INVOICE_STATUS, Tax, TaxWithholding } from '@/types';
...
interface PurchaseInvoiceFinancialInformationProps {
  className?: string;
...
export const PurchaseInvoiceFinancialInformation = ({
  className,
  subTotal,
  status,
  currency,
  ...
  loading,
  edit = true
}: PurchaseInvoiceFinancialInformationProps) => {
  const { t: tInvoicing } = useTranslation('invoicing');

  const purchaseInvoiceArticleManager = usePurchaseInvoiceArticleManager();
  ...
  const taxWithholdingAmount = React.useMemo(() => {
    if (purchaseInvoiceManager.taxWithholdingId) {
      const taxWithholding = taxWithholdings?.find((t) => t.id === purchaseInvoiceManager.taxWithholdingId);
      const taxWithholdingAmount = purchaseInvoiceManager.total * ((taxWithholding?.rate || 0) / 100);
      return ciel(taxWithholdingAmount, currency?.digitAfterComma || 3);
    }
    return 0;
  }, [
    purchaseInvoiceManager.taxWithholdingId,
    purchaseInvoiceManager.total,
    taxWithholdings,
  ...
  const discount = purchaseInvoiceManager.discount ?? 0;
  const discountType =
    purchaseInvoiceManager.discountType === DISCOUNT_TYPE.PERCENTAGE ? 'PERCENTAGE' : 'AMOUNT';
  const remaining_amount =
    (purchaseInvoiceManager.total || 0) - (purchaseInvoiceManager.amountPaid || 0) - taxWithholdingAmount;
  ...
  return (
    <div className={cn(className)}>
      ...
      <div className="flex flex-col w-full border-b">
        ...
        {purchaseInvoiceArticleManager.taxSummary.map((ts) => {
          return (
          ...
          <div className="flex items-center my-2">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                type="number"
                value={discount}
                onChange={(e) => purchaseInvoiceManager.set('discount', parseFloat(e.target.value))}
              />
               <Select
        ...
        {!controlManager.isTaxStampHidden && (
          <div className="flex items-center my-2">
            ...
            {edit ? (
              <Select
                onValueChange={(value: string) => {
                  purchaseInvoiceManager.set('taxStampId', parseInt(value));
                }}
                defaultValue={purchaseInvoiceManager.taxStampId?.toString()}>
              ...
              <div className="flex flex-col w-full">
                <Label className="ml-auto" >
                  {taxes.find((t) => (t.id = purchaseInvoiceManager.taxStampId))?.value}{' '}
      ...
      {[PURCHASE_INVOICE_STATUS.PartiallyPaid, PURCHASE_INVOICE_STATUS.Unpaid, PURCHASE_INVOICE_STATUS.Sent].includes(
        status
      ) && (
        <div>
          <div className="flex flex-col w-full">
