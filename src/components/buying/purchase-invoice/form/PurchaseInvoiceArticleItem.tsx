...
import { ArticlePurchaseInvoiceEntry, ResponseCurrencyDto, PurchaseInvoiceTaxEntry, Tax } from '@/types';
import {
  Select,
  SelectTrigger,
  SelectContent,
...
import { PurchaseInvoiceTaxEntries } from './PurchaseInvoiceTaxEntries';


interface PurchaseInvoiceArticleItemProps {
  className?: string;
  article: ArticlePurchaseInvoiceEntry;
  onChange: (item: ArticlePurchaseInvoiceEntry) => void;
...
export const PurchaseInvoiceArticleItem: React.FC<PurchaseInvoiceArticleItemProps> = ({
  className,
  article,
  onChange,
  taxes,
  ...
  const handleTaxChange = (value: string, index: number) => {
    ...
    const updatedTaxes = [...(article.articlePurchaseInvoiceEntryTaxes || [])];
    if (selectedTax) {
      updatedTaxes[index] = { tax: selectedTax };
    } else {
      updatedTaxes.splice(index, 1);
    }
    onChange({ ...article, articlePurchaseInvoiceEntryTaxes: updatedTaxes });
  };

  const handleTaxDelete = (index: number) => {
    const updatedTaxes = article.articlePurchaseInvoiceEntryTaxes?.filter((_, i) => i !== index);
    onChange({ ...article, articlePurchaseInvoiceEntryTaxes: updatedTaxes });
  };

  const handleAddTax = () => {
    if ((article.articlePurchaseInvoiceEntryTaxes?.length || 0) >= taxes.length) {
      toast.warning(tInvoicing('quotation.errors.surpassed_tax_limit'));
      return;
    }
    onChange({
      ...article,
      articlePurchaseInvoiceEntryTaxes: [...(article.articlePurchaseInvoiceEntryTaxes || []), {} as PurchaseInvoiceTaxEntry]
    });
  };

  const selectedTaxIds = article.articlePurchaseInvoiceEntryTaxes?.map((t) => t.tax?.id) || [];
                ...
                article.article?.description && (
                  <>
                    ...
                    <Textarea
                      ...
                      className="resize-none"
                      onClick={() => {}}
                      rows={3 + (article?.articlePurchaseInvoiceEntryTaxes?.length || 0)}
                    />
                  </>
                )
              )}
