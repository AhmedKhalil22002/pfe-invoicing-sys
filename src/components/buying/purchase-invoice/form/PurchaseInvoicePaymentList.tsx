...
import { ResponseCurrencyDto, PaymentPurchaseInvoiceEntry } from '@/types';
import { ciel } from '@/utils/number.utils';
import Link from 'next/link';
import { useTranslation } from 'react-i18next';

interface PurchaseInvoicePaymentListProps {
  className?: string;
  payments?: PaymentPurchaseInvoiceEntry[];
  currencies?: ResponseCurrencyDto[];
}

export const PurchaseInvoicePaymentList = ({
  className,
  payments,
  currencies
}: PurchaseInvoicePaymentListProps) => {
  const { t: tInvoicing } = useTranslation('invoicing');
  return (
    <Accordion type="multiple" className={cn(className)}>
      <AccordionItem value="purchase-invoice-list">
        <AccordionTrigger>
